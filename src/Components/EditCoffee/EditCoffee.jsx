import styles from './EditCoffee.module.css';

import {useState, useEffect} from 'react';
import {useHistory} from 'react-router-dom';

function AddCoffee(props) {
  const [coffeeName, setCoffeeName] = useState(props.coffee.name);
  const [coffeeContinent, setCoffeeContinent] = useState(props.coffee.continent);
  const [coffeeCountry, setCoffeeCountry] = useState(props.coffee.country);
  const [coffeeProcess, setCoffeeProcess] = useState(props.coffee.process);
  const [coffeePrice, setCoffeePrice] = useState(props.coffee.price);
  const [coffeeRoast, setCoffeeRoast] = useState(props.coffee.roast);
  const [coffeeDescriptor1, setCoffeeDescriptor1] = useState(props.coffee.descriptors[0]);
  const [coffeeDescriptor2, setCoffeeDescriptor2] = useState(props.coffee.descriptors[1]);
  const [coffeeDescriptor3, setCoffeeDescriptor3] = useState(props.coffee.descriptors[2]);
  const [coffeeDescription, setCoffeeDescription] = useState(props.coffee.description);
    ...
    <>
      ...
      <div className={styles.buttonDiv}>
        ...
        <div className={styles.rightButtons}>
          ...
          <button onClick={editCoffee} className={styles.primaryButton}>save</button>
        </div>
      </div>
    </>
  );
